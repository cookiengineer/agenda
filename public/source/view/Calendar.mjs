		...
		week.forEach((day) => {
			...
			if (day.tasks.length > 0) {
				...
				day.tasks.forEach((task) => {
					...
					content.push('<li>');
					content.push('<button data-view="editor" data-id="' + task.id + '" data-type="' + task.type + '" data-project="' + task.project + '">');
...
Calendar.prototype = {
	...
	render: function(task) {
		...
		if (datetime !== null) {
			renderCalendar.call(this, datetime.year, datetime.month);
		}
			...
			this.app.tasks.filter((task) => {
				...
				if (datetime !== null) {
					...
					if (isString(task.deadline) === true) {
						...
						// TODO: Figure out whether it makes sense to display unscheduled tasks
						// matches_datetime = true;

						matches_datetime = false;
